    ...
    <>
      <section className="hidden lg:flex justify-center flex-wrap gap-4 mt-6  sm:mx-[4rem] lg:mx-[12rem] 2xl:w-[800px] 2xl:mx-auto">
        {genres.map((genre, genreIndex) => (
          <button
            className={`flex gap-2.5 border-2 border-[#26295a] rounded-[.4rem] px-4 py-0.5 text-[.9rem] cursor-pointer hover:bg-[#D5A439] hover:text-[#26295a] ${
              selectGenre.includes(genre.value)
                ? "bg-[#534428] text-[#ffffff] border-[#D5A439]"
                : ""
            }`}
            value="action"
          ...
          >
            {selectGenre.includes(genre.value) ? (
              <img
                ...
                src={selectGenre.includes(genre.value) ? verifyIcon : ""}
                alt=""
              />
            ) : (
              ""
